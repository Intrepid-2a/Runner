...
def doVanBaelenTask(ID=None, hemifield=None, location=None):
    ...
    setup = localizeSetup(location=location, trackEyes=trackEyes, filefolder=eyetracking_path, filename=et_filename+str(x), task='vanbaelen', ID=ID) # data path is for the mapping data, not the eye-tracker data!
    ...
    ## instructions
    visual.TextStim(win,'Throughout the experiment you will fixate a cross that will be located at the center of the screen.   \
    It is important that you fixate on this cross at all times.\n\n You will be presented with three dots. You will have to indicate which dots were closer together.\n\n Up arrow = Top two dots were closer together.\
    \n\n Down arrow = bottom two dots were closer together.\n\n\n Press the space bar to start the experiment.', height = letter_height,wrapWidth=15, color = 'black').draw()
    win.flip()
    k = ['wait']
    ...
    spot_cart = bs_prop['cart'] # x, y cartesian coordinates of the centre (in dva)
    spot_size = bs_prop['size'] # width and height of the ellipse (in dva)
    spot      = bs_prop['spot'] # polar coordinates relative to fixation: angle (degrees) and distance (dva)
    ...
    ######
    ###### VanBaelen staircase definitions:
    ######


    eye        = ['ipsi', 'ipsi', 'ipsi', 'ipsi', 'contra', 'contra', 'contra', 'contra']
    pos_arrays = [pos_array_bsa[:]] * 2 + [pos_array_out[:]] * 2 + [pos_array_bsa[:]] * 2 + [pos_array_out[:]] * 2
    foil_type  = [1,-1] * 4
